...
public class MarkAnalysisPlotController {
	...
	@RequestMapping(value="/markAnalysis",method=RequestMethod.POST,produces = "application/json")
...
/*       GroupingCategory catg2 = new GroupingCategory();
       ...
       markAnalysis.getCategories().add(catg2); */
		...
		List<Series> improvementSeriesList = new ArrayList<Series>();
		for (Map.Entry<String, List<User>> entry : userByCategory.entrySet())
		{	
			Series series = new Series();
			series.name = entry.getKey().replace('+', ' ');
			...
			for(User user : userList){
			    Float usageMark[] =  new Float[2];
			    usageMark[0]= (float) user.getTotalUsage();
			    usageMark[1] = user.getMarks().get(test2Index).getMarks() - user.getMarks().get(test1Index).getMarks();
			    series.data.add(usageMark);
			}			
			improvementSeriesList.add(series);
		}
        ...
        model.put("listOfSeriesList", listOfSeriesList);
        model.put("improvementSeriesList", improvementSeriesList);
